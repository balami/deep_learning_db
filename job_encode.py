...
def gen_query_files():
    ...
    for queryfile in allfiles:
        ...
        with open(queryfile, 'r') as file:
            ...
            encoded_query = re.sub("(SELECT(.+?)FROM )", '', query).replace('AS ', '').replace(',WHERE ', '#').replace(
                ',AND ', '#').replace(';,', '').replace(',OR', ' OR')
...
def create_pd_df(concat_arr):
    df = pd.DataFrame(concat_arr, columns=['Query Number', 'Tables', 'Joins', 'Predicate String', 'Predicate Numeric',
                                           'Cardinality'])
    print(df)
